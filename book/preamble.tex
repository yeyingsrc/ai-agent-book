...
% ── CJK fonts (classoption=nofont ⇒ ctex fontset=none) ────────────
% Set explicitly instead of relying on ctex's platform auto-detection, which
% picks the legacy ST* families (STSong/STHeiti/STKaiti). Those exist on old
% macOS installs but NOT on current systems / GitHub macos runners, while
% Songti SC / Heiti SC below ship with every macOS.
