...
对应到 IPC 的两大范式：共享文件系统就是 Agent 世界的“共享内存”；工具调用参数和消息总线则是“消息传递”的两种形态——前者随调用同步传递，后者经中转站异步投递。
...
这套抽象在计算机科学里有个更早的名字：Actor 模型[^actor-model]——每个 Actor 持有私有状态，只通过异步消息与外界交流，并且可以创建新的 Actor。Erlang 语言把它工程化，支撑了电信系统几十年的高可用运行；多 Agent 系统可以看作 Actor 模型的 LLM 版本。这意味着操作系统和分布式系统积累的设计经验大多可以直接借用：怎么隔离、怎么通信、怎么创建和回收、崩溃了怎么恢复，接下来两小节大体就是沿这份清单展开的。类比只在一处失效：进程之间传的是字节，逐位保真；Agent 之间传的是语义，每转述一次都可能失真。这是多 Agent 系统真正的新问题，留到本章“失败模式”一节讨论。

[^actor-model]: Hewitt, C., Bishop, P., Steiger, R. *A Universal Modular ACTOR Formalism for Artificial Intelligence.* IJCAI 1973. Erlang 对 Actor 模型的工程化，见 Armstrong, J. *Making Reliable Distributed Systems in the Presence of Software Errors.* PhD thesis, KTH, 2003.
...
进程管理中还有一个经典问题同样适用于这里：回收。子进程退出后须由父进程 `wait()` 回收，否则沦为僵尸进程；父进程先退出，孤儿进程交由 init 收养。Agent 系统同样要回答：子 Agent 结束后，结果由谁回收？Manager 自身崩溃后，仍在运行的子 Agent 由谁接管？Erlang 的监督树（supervision tree）给出了成熟的答案：每个 Agent 都有明确的监督者，由监督者回收结果、处理残局，并按预设策略决定重启还是放弃——上文“从最后一个有效状态重启崩溃的子 Agent”，就是一种重启策略。
